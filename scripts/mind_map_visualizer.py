...
def create_mind_map_visualization(
    mind_map_data: Dict, output_file: str = "mind_map.png"
):
    ...
    for i, (branch, angle) in enumerate(zip(branches, angles)):
        ...
        ax.plot(
            [central_x, branch_x], [central_y, branch_y], "k-", alpha=0.5, linewidth=1
        )
...
def create_text_summary(mind_map_data: Dict, output_file: str = "mind_map_summary.txt"):
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        total_chars = sum(
            branch["content_length"] for branch in mind_map_data["branches"]
        )
...
def create_enhanced_mind_map(
    mind_map_data: Dict, output_file: str = "enhanced_mind_map.png"
):
    ...
    for i, (branch, angle) in enumerate(zip(branches, angles)):
        ...
        # Draw connection line
        ax.plot(
            [central_x, branch_x], [central_y, branch_y], "k-", alpha=0.6, linewidth=2
        )
    ...
    plt.title(
        "Enhanced Safari Reading List Mind Map", fontsize=18, fontweight="bold", pad=30
    )
    plt.savefig(output_file, dpi=300, bbox_inches="tight")
    plt.close()
